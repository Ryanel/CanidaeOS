...
int elf_check_executable(elf64_header_t *elf_header) {
    ...
    if (elf_header->e_machine != 0x3E) { return 0; }
    if (elf_header->e_type != 2) { return 0; }
    return 1;
}
...
loaded_kernel_info_t awd_load_kernel(uint32_t baseAddress) {
    ...
    for (unsigned int i = 0; i < elf_header->e_phnum; i++) {
        ...
        if (pheader->p_type != 1) { continue; }

        uint32_t ph_file_address = baseAddress + pheader->p_offset;

        if (kernel_info.virt_start > pheader->p_vaddr) { kernel_info.virt_start = pheader->p_vaddr; }
        if (kernel_info.phys_start > ph_file_address) { kernel_info.phys_start = ph_file_address; }
...
#ifdef DEBUG_LOG
        console_log("elf64", "LOAD<%d> [p]0x%08X -> [p]0x%08X [v]", i, ph_file_address, dest_address);
