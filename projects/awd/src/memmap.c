...
void memmap_phys_add(awd_physmemmap_t* x) {
    ...
    while (srcrt->next != 0) { srcrt = (awd_physmemmap_t*)srcrt->next; }

    srcrt->next = (uint32_t)(x);
}
awd_physmemmap_t* memmap_phys_create(char type, uint32_t startAddress_lo, uint32_t startAddress_hi, uint32_t size_lo,
                                     uint32_t size_hi) {
