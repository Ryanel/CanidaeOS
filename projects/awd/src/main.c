...
void awd_main(uint32_t mb_magic, const multiboot_info_t* const mb_info) {
    ...
    console_log("awd", "African Wild Dog v. 0.1\n");
    ...
    console_log("awd", "Kernel located at: 0x%08X -> 0x%08X\n", kernel_module->mod_start, kernel_module->mod_end);

    // Load the kernel into memory
    loaded_kernel_info_t kernel = awd_load_kernel(kernel_module->mod_start);

    // Print out kernel start
    uint32_t vaddr_lo = kernel.virt_start & 0xFFFFFFFF;
    uint32_t vaddr_hi = kernel.virt_start >> 32;
    console_log("kernel", "Starts at: 0x%08X%08X\n", vaddr_hi, vaddr_lo);

    map_kernel(kernel);

    console_log("awd", "Entering 64-bit mode and executing the kernel\n");
